...
void TSL2591_Enable(void) {
    TSL2591_Write_Byte(ENABLE_REGISTER,
                       ENABLE_AEN | ENABLE_POWERON); // TSL2591_Write_Byte(0xA0,0x93) | ENABLE_NPIEN
                                                     // | ENABLE_AEN | ENABLE_NPIEN | ENABLE_AIEN
}
...
void TSL2591_Disable(void) {
    TSL2591_Write_Byte(ENABLE_REGISTER, ENABLE_POWEROFF);
}
...
void TSL2591_Set_Gain(UBYTE Gain) {
    UBYTE control = 0;
    if (Gain == LOW_AGAIN || Gain == MEDIUM_AGAIN || Gain == HIGH_AGAIN || Gain == MAX_AGAIN) {
        control = TSL2591_Read_Byte(CONTROL_REGISTER); // lee A1
        control &= 0xCf;                               // 0b11001111
        control |= Gain;
        TSL2591_Write_Byte(CONTROL_REGISTER, control); // escribe 0xA1 y 0X11
        TSL2591_Gain = Gain;
    } else {
